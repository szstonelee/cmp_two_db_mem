		...
		switch (*p)
		{
			...
			case 'b':
				{
					...
					if (MyProcPid == PostmasterPid)
						...
						backend_type_str = "unknown process type";
			...
			case 'x':
				if (padding != 0)
					...
					appendStringInfo(buf, "%*u", padding, 0);
				else
					// appendStringInfo(buf, "%u", GetTopTransactionIdIfAny());
					appendStringInfo(buf, "%u", 0);
				break;
			case 'e':
				if (padding != 0)
			...
			case 'Q':
				if (padding != 0)
					...
					appendStringInfo(buf, "%*lld", padding, (long long)0);
				else
					// appendStringInfo(buf, "%lld",
					// 				 (long long) pgstat_get_my_query_id());
					appendStringInfo(buf, "%lld", (long long)0);
				break;
			default:
...
write_csvlog(ErrorData *edata)
{
	...
	if (MyProcPid == PostmasterPid)
		...
		appendCSVLiteral(&buf, "unknown process type");
